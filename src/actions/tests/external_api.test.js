...
describe("test actions/external_api", () => {
  ...
  const mockBitumDataPath = `https://testnet.bitum.io/api/address/${FAKE_ADDRESS}/raw`;
  const mockBitumDataResponseLackingConfirmation = [
    {
      size: 251,
  ...
  const mockBitumDataResponseComplete = [
    {
      size: 251,
      txid: "997a39549ed7ec5cb64f90753c6c69c6db8a84b06c2908cc0aea3c4a4d5e0805",
      version: 1,
  ...
  test("test verify user payment action", async () => {
    ...
    setGetSuccessResponse(mockBitumDataPath, {}, mockBitumDataResponseComplete);
    ...
    setGetSuccessResponse(
      mockBitumDataPath,
      {},
      mockBitumDataResponseLackingConfirmation
    );
    await store.dispatch(ea.verifyUserPayment.apply(null, params));
    expect(store.getActions()).toEqual([
      {
  ...
  test("pay with faucet action", async () => {
    const faucetPath = "https://faucet.bitum.io/requestfaucet";
