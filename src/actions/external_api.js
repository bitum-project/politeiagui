...
export const verifyUserPayment = (address, amount, txNotBefore) => dispatch => {
  // Check bitumdata first.
  return external_api
    .getPaymentsByAddressBitumdata(address)
    .then(response => {
      if (response === null) {
        return null;
      }
      return checkForPayment(
        checkBitumdataHandler,
        response,
        address,
        amount,
        txNotBefore
    ...
    .catch(() => {
      // Failed to fetch from bitumdata.
      return null;
    })
    .then(txn => {
      if (txn) {
...
export const getLastBlockHeight = () => (dispatch, getState) => {
  ...
  external_api
    .getHeightByBitumdata(isTestnet)
    .then(response => {
